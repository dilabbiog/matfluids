...
elseif nargin == 5 && strcmpi(varargin{1}, 'fracstep')
    ...
    for k1 = 1:n-1
        ...
        Xfrac = permute(interp1([0 1], X, 0:1/N:1), [2 3 1]);
        Yfrac = permute(interp1([0 1], Y, 0:1/N:1), [2 3 1]);
        Ufrac = permute(interp1([0 1], U, 0:1/N:1), [2 3 1]);
        Vfrac = permute(interp1([0 1], V, 0:1/N:1), [2 3 1]);
        for k2 = 1:N
            ...
            K1v = interp2(Xfrac(:,:,k2), Yfrac(:,:,k2), Vfrac(:,:,k2), ...
                          A{c}.X, A{c}.Y, 'cubic');
            K2u = interp2(Xfrac(:,:,k2), Yfrac(:,:,k2), Ufrac(:,:,k2), ...
                          A{c}.X + 0.5*(dt/N)*K1u, ...
                          A{c}.Y + 0.5*(dt/N)*K1v, 'cubic');
            K2v = interp2(Xfrac(:,:,k2), Yfrac(:,:,k2), Vfrac(:,:,k2), ...
                          A{c}.X + 0.5*(dt/N)*K1u, ...
                          A{c}.Y + 0.5*(dt/N)*K1v, 'cubic');
            K3u = interp2(Xfrac(:,:,k2), Yfrac(:,:,k2), Ufrac(:,:,k2), ...
                          A{c}.X + 0.5*(dt/N)*K2u, ...
                          A{c}.Y + 0.5*(dt/N)*K2v, 'cubic');
            K3v = interp2(Xfrac(:,:,k2), Yfrac(:,:,k2), Vfrac(:,:,k2), ...
                          A{c}.X + 0.5*(dt/N)*K2u, ...
                          A{c}.Y + 0.5*(dt/N)*K2v, 'cubic');
            K4u = interp2(Xfrac(:,:,k2+1), Yfrac(:,:,k2+1), ...
                          Ufrac(:,:,k2+1), A{c}.X + (dt/N)*K3u, ...
                          A{c}.Y + (dt/N)*K3v, 'cubic');
            K4v = interp2(Xfrac(:,:,k2+1), Yfrac(:,:,k2+1), ...
                          Vfrac(:,:,k2+1), A{c}.X + (dt/N)*K3u, ...
                          A{c}.Y + (dt/N)*K3v, 'cubic');
